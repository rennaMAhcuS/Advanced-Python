...
def randomly_remove_walls():
    total_cells = rows * cols
    cells_to_update = total_cells * 30 // 100  # calculate 20% of total cells

    for _ in range(cells_to_update):
        random_cell = random.choice(random.choice(grid))
        random_cell.remove_wall(3)  # remove the left wall
...
while not done:
    ...
    elif len(stack) == 0 and not maze_generated:
        maze_generated = True
        # randomly_remove_walls()
